...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    # Converting 'holiday_date' column in holidays DataFrame to datetime and comparing it with the order dates.
    holidays["date"] = pd.to_datetime(holidays["date"]).dt.date

    # Adding the 'holiday' column to the result, indicating if each date is a holiday.
    order_purchase_ammount_per_date["holiday"] = order_purchase_ammount_per_date["order_purchase_timestamp"].isin(holidays["date"])

    # Creating the final DataFrame for the output with the required columns.
    result_df = order_purchase_ammount_per_date[["order_count", "order_purchase_timestamp", "holiday"]].rename(columns={'order_purchase_timestamp': 'date'})
