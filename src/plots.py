...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    plt.figure(figsize=(6, 5))  # Create a new figure with specified size
    sns.scatterplot(data=df, x='product_weight_g', y='freight_value')  # Create a scatter plot with product weight on the x-axis and freight value on the y-axis
    plt.title("freight_value vs product_weight_g")  # Set the title of the plot
    plt.xlabel("product_weight_g")  # Label the x-axis
    plt.ylabel("freight_value")  # Label the y-axis
    plt.legend(["freight_value"])  # Add a legend to the plot
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    ax1.plot(df['date'], df['order_count'], label='Order amount per day', color='g')

    # Mark holiday days
    for i, is_holiday in enumerate(df['holiday']):
        if is_holiday:
            ax1.axvline(x=df['date'].iloc[i], color='b', linestyle=':', label='Holiday' if i == 0 else "")

    # Configure labels and title
    ax1.set_title("Order amount per day with holidays")
    ax1.set_xlabel("Date")
    ax1.set_ylabel("Order amount per day")
    ax1.legend()
