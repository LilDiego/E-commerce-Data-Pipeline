...
def get_public_holidays(public_holidays_url: str, year: str) -> DataFrame:
    ...
    # Realizamos la solicitud GET a la API para obtener los días festivos
    response = requests.get(url)

    #Verificamos respuesta exitosa con codigo 200
    if response.status_code == 200:
        #Convertimos el JSON en un DataFrame de pandas
        data = response.json()

        df = DataFrame(data)

        #Eliminamos las columnas no deseadas        
        df.drop(['types','counties'],axis=1, inplace=True)

        #Convertimos la columna date a un formato de fechas de pandas

        df['date'] = to_datetime(df['date'])
    else:

        #Si la solicitud falla, lanzamos SystemExit para detener la ejecución
        raise SystemExit
